...
var midiUtils = require('./midi-utils'),
	...
	isProgramAftertouchEvent = midiUtils.isProgramAftertouchEvent;
...
var constants = require('./midi-constants'),
    ...
    CHANNEL_MASK = constants.CHANNEL_MASK,
...
function generateEvent(MetaEventClass, eventCode, subtype, deltaTime, dataBytes, track) {
    return new MetaEventClass({
        ...
        delta: deltaTime,
        track: track,
		// TODO: how to not pass in dataBytes
        dataBytes: dataBytes
    });
}

function generateTempoEvent(eventCode, subtype, deltaTime, dataBytes, track) {
    return new MidiMetaTempoEvent({
        code: eventCode,
        subtype: subtype,
        delta: deltaTime,
        track: track,
		tempo: parseByteArrayToNumber(dataBytes)
    });
}

var processMetaEvent = dispatch(
    generateMetaEventGuard(TRACK_NAME_META_EVENT, partial(generateEvent, MidiMetaTrackNameEvent)),//0x03, processTrackName),
    generateMetaEventGuard(END_OF_TRACK_META_EVENT, partial(generateEvent, MidiMetaEndOfTrackEvent)),
    generateMetaEventGuard(TEMPO_META_EVENT, generateTempoEvent),
    generateMetaEventGuard(TIME_SIG_META_EVENT, partial(generateEvent, MidiMetaTimeSignatureEvent)),
    generateMetaEventGuard(INST_NAME_META_EVENT, partial(generateEvent, MidiMetaInstrumentNameEvent)),
    generateMetaEventGuard(KEY_SIGNATURE_META_EVENT, partial(generateEvent, MidiMetaKeySignatureEvent)),
    generateMetaEventGuard(SMPTE_OFFSET_META_EVENT, partial(generateEvent, MidiMetaSmpteOffsetEvent)),
...
var processProgramAtertouchEvent = dispatch(
    generateChannelEventGuard(CHANNEL_MASK, partial(generateChannelEvent, MidiChannelEvent)),
    function _noMatch(eventCode/*, deltaTime, dataBytes */) {
        throw new Error('unknown channel event "' + toHex(eventCode) + '"');
    }
);
...
function parseEvents(midiBytes, track) {
    ...
    while (midiBytes.length) {
		...
		if (!isValidEventCode(midiBytes[0])) {
			// assume we have a running status
		...
		if (isProgramAftertouchEvent(eventCode)) {
            midiEvent = processProgramAtertouchEvent(eventCode, deltaTime, dataBytes, track);
            midiBytes = midiBytes.slice(1);
		} else if (isMetaEvent(eventCode)) {
